...
const KINDS = [
  { due: 'notes_due_at', sent: 'notes_sent_at', count: 'reminder_count',
    what: 'Class notes', how: 'Please send them here as a PDF/document.' },
  { due: 'test_result_due_at', sent: 'test_result_sent_at', count: 'test_reminder_count',
    what: 'Sunday test result', how: 'Please send it here and include #result in the message.' },
].map((k) => ({
  ...k,
  select: db.prepare(`
    SELECT c.id, c.${k.due} AS due, c.${k.count} AS n, t.wa_jid, t.name AS teacher, s.name AS student
    ...
    for (const r of k.select.all(new Date(now).toISOString())) {
      ...
      try {
        await gowa.sendText(cfg.teacherDevice, r.wa_jid,
          `Reminder ${r.n + 1}/2: ${k.what} for ${r.student} (class #${r.id}) is overdue. ${k.how}`);
        k.bump.run(r.id);
        if (r.n + 1 >= 2) {
