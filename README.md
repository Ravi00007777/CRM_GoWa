...
public/index.html    admin dashboard (served at /)
src/db.js            schema
...
curl -H "$T" -H "$J" localhost:8080/admin/students -d '{"name":"Diya","parent_phone":"+91 87654 32109","parent_payment_email":"parent@example.com","grade":"8","teacher_id":1}'
curl -H "$T" -H "$J" localhost:8080/admin/classes  -d '{"student_id":1,"held_at":"2026-09-14T16:30:00+05:30","meet_link":"https://meet.google.com/abc-defg-hij"}'
...
Open <http://localhost:8080> while `npm start` runs and sign in with `ADMIN_TOKEN`.
...
A student has one teacher at a time; a teacher can have many students, and a parent can have several children. For each message, in order:

1. Starts with `#tag` (e.g. `#diya homework done?`) -> that student; the tag is removed.
2. Swipe-reply to an earlier relayed message -> the same conversation (refused if that assignment has ended).
3. Sender has exactly one assigned student -> that student.
4. Otherwise nothing is sent and the sender is asked "Which student? Start your message with #diya or #rohan."

`#list` replies with the sender's students/children and their tags. Tags are unique per teacher and per parent; `#result` and `#list` are reserved. Teachers see `Student Diya (#diya): ...`; parents see `Teacher Asha (for Diya): ...`.
...
| Document (PDF etc.) | Caption redacted. File relayed as `class-<id>.<ext>`, so the original filename never leaves the system. From a teacher, it sets `notes_sent_at` on the latest class that has started |
...
| Sender with no assigned student | The sender is told so and the admin is alerted |

Redaction removes phone-shaped numbers (10 or more digits, even with spaces, dashes, dots or parentheses), emails, `wa.me` / WhatsApp invite links and JIDs. Just before sending, every outbound message is checked once more and refused if it contains both an email and a number/JID.
